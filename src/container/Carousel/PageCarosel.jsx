...
const PageCarosel = ({ data, title }) => {
  ...
  const responsive = title.length !== 0 ? caroselCardResponsive : typeCardResponsive;

  const [active, setActive] = useState(0);
  
  const leftClick = () => setActive(active - 1);
  const rightClick = () => setActive(active + 1);
  ...
  const items = data.map((item) => {
    if (title.length !== 0) {
      return <CaroselCard item={item} />;
    } else {
      return <TypeCard item={item} />;
    }
  });
  ...
  return (
    <>
      <div className="w-11/12 my-4 md:my-8 mx-auto shadow-[rgba(0,_0,_0,_0.1)_0px_30px_90px] bg-slate-200">
        {title.length!==0 && <h2 className="md:text-4xl font-bold m-2 p-3">{title}</h2>}
